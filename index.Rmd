...
present, `master` is chosen by default. Click on `Create new branch`.
...
Click on the `Pull request` button in GitHub Desktop, enter a summary and description
of the proposed changes and why you did them. Then click on `Send pull request`. 
...
`pcaCars` repositor of your online GitHub account.
...
Enter a comment about the pull request (optional) and click on `Confirm merge`.
...
delete the `clustering` branch as it is no longer needed because these changes
...
Notice the `Git` tab next to the `History` tab in the top right panel. It is empty
...
Describe the commit and click on `Commit to master`. 
...
`Sync` the _local_ repository with the _remote_.
...
`Publish` button in GitHub Desktop. 
...
Create a `gh-pages` branch and click on `Create new branch`. 
...
Commit the changes in GitHub Desktop. Click on `Commit to gh-pages`. 
...
Click on `Publish` to _push_ your changes to your online GitHub account. 
...
After entering a description of the repository click on `Publish cv`.
